...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/07/03 15:15:05 by aljigmon          #+#    #+#             */
/*   Updated: 2019/07/05 16:11:13 by aljigmon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	option_flag_width(t_p *var, int size_nbr, unsigned long long int n)
{
	...
	if (!var->flag['-'] && var->flag['#'] && !var->flag['0'])
		sum += write(1, "0X", 2);
	return (sum += !var->flag['-'] ? print_value_xs(n, var, 0) : 0);
}
...
int			func_big_x(t_p *var)
{
	...
	if ((var->flag['#'] || var->flag['-']
		...
		sum = option_hashtag_xs(*ptr, var, 0);
	else if (var->pr && var->wd)
		sum = w_p_xs(var, size_nbr, *ptr, 0);
	else if (var->wd)
		sum = wd_xs(var, size_nbr, *ptr, 0);
	else if (var->pr)
		sum = pr_xs(var, size_nbr, *ptr, 0);
	else
		sum = print_value_xs(*ptr, var, 0);
	return (sum);
}
